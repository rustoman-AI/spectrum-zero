...
export function updateEnemies(dt) {
  ...
  for (let i = 0; i < pool.length; i++) {
    ...
    if (e.propulsion === 'oared') {
      ...
      // Hull rock: ±2 degrees in time with strokes
      e.mesh.rotation.z = Math.sin(e.oarPhase) * 0.035; // ~2 degrees
      e.driftX = 0;
    } else {
      // Sailed: gentle lateral drift (slow sinusoid)
      e.oarPhase += dt * 1.2;
      e.driftX = Math.sin(e.oarPhase) * 0.8; // gentle sway
      e.mesh.rotation.z = Math.sin(e.oarPhase * 0.7) * 0.02; // very slight roll
    }
