...
export const GOD_ABILITIES = {
  zeus: {
    ...
    costs: [
      ...
      { faith: 25, gold: 5 },         // #2+: constant. Low + reachable now that the
                                      // Faith SOURCE (Helios +8, was +15) is trimmed
                                      // instead of the price — lands ~4 faith casts
                                      // committed, ~1 casual (the actual complaint).
    ]
  },
  poseidon: {
    ...
    costs: [
      ...
      { faith: 30, gold: 8 },         // #2+: constant — a touch dearer than Zeus.
    ]
  },
  ...
  helios: {
    ...
    faithGain: 8,          // +8 Faith over the 5s flare (+1.6/sec). Reduced 15->8:
                           // Helios is the real Faith engine, so trimming the
                           // grant at the SOURCE lets the repeat cost stay low
                           // (reachable) without flooding into too many casts.
                           // The stun + shield-burn remain the reason to cast it;
                           // Faith is a bonus, not the point.
