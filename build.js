  ...
  layer.addEventListener('pointerdown', function() {
    ...
    if (playPromise && playPromise.then) {
      playPromise.then(function() {
        updateDebugInfo('video playing (with audio)');
      }).catch(function(err) {
        updateDebugInfo('play() rejected: ' + err.name + ' — trying muted');
        // Fallback: play muted, show unmute button
        video.muted = true;
        var mutePromise = video.play();
        if (mutePromise && mutePromise.then) {
          mutePromise.then(function() {
            updateDebugInfo('video playing (muted fallback)');
            // Show unmute tap target
            var unmBtn = document.createElement('div');
            unmBtn.textContent = '\uD83D\uDD0A Tap for sound';
            unmBtn.style.cssText = 'position:absolute;top:10%;color:#fff;font:bold 14px monospace;background:rgba(0,0,0,0.6);padding:8px 16px;border-radius:4px;cursor:pointer;z-index:100000;';
            layer.appendChild(unmBtn);
            unmBtn.addEventListener('pointerdown', function(e) {
              e.stopPropagation();
              video.muted = false;
              unmBtn.remove();
              updateDebugInfo('unmuted by user');
            });
          }).catch(function(err2) {
            updateDebugInfo('muted play also failed: ' + err2.name);
            startGame();
          });
        }
      });
